...
export default function Page() {
  ...
  return (
    <div className={`fixed w-screen h-dvh font-[600] text-[14px] overflow-hidden`}>
      <div className="relative flex flex-col justify-center min-h-dvh ">
      ...
      <div className={`
        ${nowArea === 'bottomArea' ? 'top-0' : 'top-[100dvh]'}
        absolute left-0 w-full h-dvh transition-[top] duration-700 ease-in-out z-30`}>
        <CommercialMain />
      </div>

      <div className={`
        ${nowArea === 'topArea' ? 'top-0' : 'top-[-100dvh]'}
        absolute left-0 w-full h-dvh transition-[top] duration-700 ease-in-out z-30`}>
        <PersonalMain />
      </div>
    </div>
